...
![Stored(저장형) XSS 취약점 점검](../../images/10_webapp/webapp-006.jpg "Stored(저장형) XSS 취약점 점검")

- 입력값이 HTML 상에 렌더링되어 스크립트가 삽입된 URL 접근 시 스크립트가 동작하는 경우(Reflected XSS)
![Reflected(반사형) XSS 취약점 점검](../../images/10_webapp/webapp-008.jpg "Reflected(반사형) XSS 취약점 점검")
