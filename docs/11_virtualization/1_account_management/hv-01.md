...
1. 호스트 > 관리 > 시스템 > 고급 설정으로 이동
1. UserVars.HostClientSessionTimeout 설정이 600초(10분)로 설정되어 있는지 확인
    ![웹 콘솔 Session Timeout 설정 확인](../../images/11_virtualization/virtual-005.jpg "웹 콘솔 Session Timeout 설정 확인")

1. 600초(10분) 이하로 설정되어 있지 않은 경우 [옵션 편집]을 클릭하여 아래와 같이 수정
    ![옵션 값 수정](../../images/11_virtualization/virtual-007.jpg "옵션 값 수정")
...
1. 호스트에 접속
1. echo $TMOUT 명령어를 이용하여 사용자 Shell Session Timeout 설정 확인
```
$ echo $TMOUT
```
1. Session Timeout 10분을 초과하는 경우 아래 두 라인 추가
```bash
$ vi /etc/profile 
```
```
...
1. Controller VM에 접속하여 SSH Idle Timeout 설정 확인
    ...
    ```
1. vi 편집기를 이용하여 SSH Idle Timeout을 10분으로 설정
    ```
...
1. Session Timeout 설정 적용
    - Settings > UI Settings > Security Setting의 SESSION TIMTOUT 설정을 10분으로 설정

    ![옵션 값 수정](../../images/11_virtualization/virtual-009.jpg "옵션 값 수정")
