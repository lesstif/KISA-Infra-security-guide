...
![원격 데스크톱 방화벽 규칙 확인](../../images/08_dbms/d-10.jpg "원격 데스크톱 방화벽 규칙 확인")

**Step 2) DB서버에 접근 가능한 특정 IP 지정**

`원격 데스크톱 – 사용자 모드(TCP-In)/사용자 모드(UDP-In) 속성 > 영역 > 원격 IP주소 > 다음 IP주소 > 추가 > IP주소 입력`

![원격 데스크톱 – 사용자 모드(TCP-In) 속성](../../images/08_dbms/d-10.jpg "원격 데스크톱 – 사용자 모드(TCP-In) 속성")

![원격 데스크톱 – 사용자 모드(UDP-In) 속성](../../images/08_dbms/d-10.jpg "원격 데스크톱 – 사용자 모드(UDP-In) 속성")
...
`$Altibase_HOME/conf/altibase.properties`의 IP Access Control Lists에서 내부 정책에 맞게 수정
...
![altibase.properties 파일](../../images/08_dbms/d-10.jpg "altibase.properties 파일")
...
![postgres.conf 파일](../../images/08_dbms/d-10.jpg "postgres.conf 파일")
...
![pg_hba.conf 파일](../../images/08_dbms/d-10.jpg "pg_hba.conf 파일")

**Step 3) USER에 접근 허용 '계정명'과 CIDR-ADDRESS에 접속을 '허용할 IP' 설정**

!!! info "참고"
    *   PostgreSQL은 기본 설치 시 외부에서 접속할 수 없음
    *   IP 접근 제한 설정 시 `postgresql.conf`와 `pg_hba.conf` 두 개의 설정 파일이 연계되어 있으므로 하나의 파일이라도 설정이 잘못되어 있는 경우 DB 접속이 불가능 할 수 있음
...
LSNR_INVITED_IP의 최대 길이는 255자이며, 256자 이상의 IP주소를 설정할 경우에는 `LSNR_INVITED_IP_FILE`을 사용 (특정 IP주소를 기재한 파일의 절대 경로를 LSNR_INVITED_IP_FILE에 입력)
