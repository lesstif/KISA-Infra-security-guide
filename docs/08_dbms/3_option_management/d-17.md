...
!!! info "다음 정적뷰를 통해 감사 기록 조회가 가능"

DBA_AUDIT_TRAIL (SELECT * FROM dba_audit_trail;) USER_AUDIT_TRAIL (SELECT * FROM user_audit_trail;) 또는 AUDIT_TRAIL=OS AUDIT_FILE_DEST=/home/Tibero/audit/audit_trail.log AUDIT_FILE_SIZE=10M 위와 같이 설정하면 "/home/Tibero/audit/audit_trail.log"에 최대 10MB의 크기로 감사 기록이 저장됨

!!! info "감사 파일이 있는 디렉터리에는 일반 사용자는 접근할 수 없도록 설정"
