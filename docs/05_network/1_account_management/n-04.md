...
!!! info "로그인 실패 임계값"
    시스템에 로그인 시 몇 번의 로그인 실패 이후 로그인을 차단할 것인지
...
Router# show login 로그인 실패 임계값 설정 확인

**Step 3) 로그인 실패 임계값 초과 후 계정 잠금 설정**

login block-for 설정 Router# config terminal Router(config)# login block-for <계정 잠금 시간> attempts <로그인 실패 횟수(default: 3)> within <로그 인 실패 허용 시간 범위(default: 15)>
