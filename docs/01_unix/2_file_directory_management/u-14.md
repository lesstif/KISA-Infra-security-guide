...
!!! info "환경변수"
    프로세스가 컴퓨터에서 동작하는 방식에 영향을 미치는 동적인 값들의 집합으로 PATH
...
!!! info "/etc/profile 파일, root 계정, 일반 사용자 계정의 환경설정 파일을 순차적으로 검색하여 확인"
...
PATH=$PATH:$HOME/bin:<상대 경로> 또는 상대 경로 삭제 Shell 종류별 환경설정 파일 Bourne Shell(sh) /etc/profile, $HOME/.profile C Shell(csh) /etc/csh.cshrc, /etc/csh.login, $HOME/.cshrc, $HOME/.login Korn Shell(ksh) /etc/profile, $HOME/.profile, $HOME/.kshrc Bash Shell(bash) /etc/profile, $HOME/.bash_profile, $HOME/.bashrc, /etc/bash.bashrc
