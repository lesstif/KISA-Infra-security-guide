...
!!! info "NIS 주 서버는 정보표를 소유하여 NIS 대응 파일들로 변환하고, 이 대응 파일들이 네트워크를 통해"

제공됨으로써 모든 컴퓨터에 정보가 갱신되도록 함. 네트워크를 통한 공유로부터 관리자와 사용자들에게 일관성 있는 시스템 환경을 제공함
...
!!! info "Redhat 계열 리눅스는 RHEL 8 버전부터 NIS(yp rpms) 패키지가 제거되었음"
...
#ypserv:2:wait:/usr/lib/netsvc/yp/ypserv >> /dev/console 2>&1 #ypbind:2:wait:/usr/lib/netsvc/yp/ypbind >> /dev/console 2>&1
...
NIS_MASTER_SERVER=0 NIS_SLAVE_SERVER=0 NIS_CLIENT_SERVER=0 NIS 관련 서비스 데몬 설명 ypserv master와 slave 서버에서 실행되며 클라이언트로부터의 ypbind 요청에 응답 ypbind 모든 NIS 시스템에서 실행되며 클라이언트와 서버를 바인딩하고 초기화함 rpc.yppasswdd 사용자들이 비밀번호를 변경하기 위해 사용 ypxfrd NIS 마스터 서버에서만 실행되며 고속으로 NIS 맵 전송 rpc.ypupdated NIS 마스터 서버에서만 실행되며 고속으로 암호화하여 NIS 맵 전송
