...
!!! info "tftp"
    파일 전송을 위한 프로토콜로서 FTP 서비스보다 구조가 단순하며 적은 양의 데이트를 보낼 때

사용됨. 주로 원격의 부팅 파일을 불러오거나 설치 프로세스를 시작하기 위한 초기 데이터 호출 용도로 사용. 서비스 사용 시 인증 절차가 없어 보안에 취약함

!!! info "talk"
    사용자가 시스템에 원격으로 연결하여 다른 시스템에 로그인하고 있는 사용자와 대화 세션을
...
#tftp   dgram  udp  nobody   /usr/sbin/tftpd  tftpd -n #talk   stream  tcp  wait  root    /usr/sbin/talkd  talkd #ntalk  dgram  udp  wait  root    /usr/sbin/talkd  talkd
...
!!! info "Redhat 계열 리눅스는 RHEL 7 버전부터 talk 패키지가 제거되었음"
...
inetd –c 서비스 (포트) 설명 TFTP (69) 파일 전송을 위한 프로토콜로써 FTP 서비스보다 구조가 단순하며 적은 양의 데이터를 보낼 때 사용됨. 주로 원격의 부팅 파일을 불러오거나 설치 프로세스를 시작하기 위한 초기 데이터 호출 용도로 사용함. 서비스 사용 시 인증 절차가 없어 보안에 취약함 TALK (517) 사용자가 시스템에 원격으로 연결하여 다른 시스템에 로그인하고 있는 사용자와 대화 세션을 시작할 수 있음 NTALK (518) 서로 다른 시스템과 채팅을 가능하게 하는 서비스
